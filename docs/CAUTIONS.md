...
- **同角色多人就是備援鏈，不同角色不是。** `preference` 較高者先執行，停用、額度
  耗盡或路由不相容時才選下一位同角色 Agent。沒有同角色替補會在准入或恢復時明確
  阻擋；除非 PM／人類留下 explicit override，不會因「反正能跑」就偷偷交給別的角色。
- **重活要宣告自己的時間預算。** 預設 3600 秒；50 分鐘以上的階段（3D 生成、
  大型最佳化）不設 `timeout_s` 就會在整點被殺，整輪工作蒸發。
  內建網頁流程的瀏覽器 precheck 與 E2E 已明確給 7200 秒；複製後若測試更久仍須調高。
...
- **分支審查畫面是本機目標快照，不是合併承諾。** 人在看 diff 的期間遠端主線可能
  已前進；所以「合併並驗證」一定會重新 fetch、merge、跑 pre-deploy gate、原子 push
  並核對遠端 SHA。畫面可供決策，但不能取代這條交付閘道。
...
- **背景生成只能走 Bastet claim。** 設有 `async_status_path` 的資源，Agent 必須在
  結束前向 `/v1/media/claims` 登記 task id 與 worktree 相對目的地；Bastet 會持久輪詢、
  落檔並自動重跑核驗。未設定者仍須前景輪詢到檔案落地；自行啟動背景子行程會在
  headless run 結束時被回收。
- **下載主機與大小要明確縮限。** 狀態 API 憑證不會送到結果 URL；結果只准資源主機
  或 `async_download_hosts`，不跟 redirect，並受 `async_max_bytes`（主機硬上限 500 MiB）
  約束。不要把短效簽名 URL 寫入 log；領取紀錄只保存狀態與內容雜湊。
...
- 輪替或刪除後的 Bastet-managed 憑證檔不會立即刪除。請在管理頁先預覽再清理；只有
  滿 24 小時、符合 Bastet 隨機命名且已不被任何資源／頻道引用的普通檔案會被永久刪除。
  自有 `file:` 路徑、任意檔案與 symlink 永遠不在候選清單。
- 定期工作流採標準五欄 cron 與 IANA 時區。排程只負責產生一般任務，不會繞過
  工作流閘道、資源授權或維護圍欄；前次任務仍在執行時，該次 occurrence 會記為
  `skipped`，不會堆積重疊任務。
